...
is deployed to production. Apart from that, image promotion will also cut deployment time shorter by not doing a rebuild; for example, if the build is carried out
...
This means, if we have an environment variables scheme as follows:
...
`mupinnn/inject-fe` is on the list.
...
## Creating a helper function to access the variables
...
After that, you'll get this kind of results:
...
Of course, this labeling and tagging doesn't require another build process, so the above process is still relevant. Build once, multiple environments, deploy anywhere.
